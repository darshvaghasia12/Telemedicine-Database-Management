...
var UserTrackingService = (function () {
    function UserTrackingService(analytics, zone, platformId) {
        ...
        if (!isPlatformServer(platformId)) {
            zone.runOutsideAngular(function () {
                _this.disposable = from(import('firebase/auth')).pipe(switchMap(function () { return analytics.app; }), map(function (app) { return app.auth(); }), switchMap(function (auth) { return new Observable(auth.onAuthStateChanged.bind(auth)); }), switchMap(function (user) { return analytics.setUserId(user ? user.uid : null); }), runOutsideAngular(zone)).subscribe();
            });
        }
    }
