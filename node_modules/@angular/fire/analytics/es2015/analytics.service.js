...
let UserTrackingService = class UserTrackingService {
    constructor(analytics, zone, platformId) {
        if (!isPlatformServer(platformId)) {
            zone.runOutsideAngular(() => {
                this.disposable = from(import('firebase/auth')).pipe(switchMap(() => analytics.app), map(app => app.auth()), switchMap(auth => new Observable(auth.onAuthStateChanged.bind(auth))), switchMap(user => analytics.setUserId(user ? user.uid : null)), runOutsideAngular(zone)).subscribe();
            });
        }
    }
    ngOnDestroy() {
