(function (global, factory) {
    ...
    var AngularFireRemoteConfig = (function () {
        function AngularFireRemoteConfig(options, nameOrConfig, settings, defaultConfig, zone) {
            this.zone = zone;
            var remoteConfig$ = rxjs.of(undefined).pipe(operators.switchMap(function () { return zone.runOutsideAngular(function () { return rxjs.empty(); }); }), operators.map(function () { return fire._firebaseAppFactory(options, zone, nameOrConfig); }), operators.map(function (app) { return app.remoteConfig(); }), operators.tap(function (rc) {
                if (settings) {
            ...
            }), operators.startWith(undefined), fire.runOutsideAngular(zone), operators.shareReplay({ bufferSize: 1, refCount: false }));
            ...
            var existing$ = loadedRemoteConfig$.pipe(operators.switchMap(function (rc) { return rc.activate().then(function () { return rc.getAll(); }); }), filterOutDefaults);
            var fresh$ = loadedRemoteConfig$.pipe(operators.switchMap(function (rc) { return zone.runOutsideAngular(function () { return rc.fetchAndActivate().then(function () { return rc.getAll(); }); }); }), filterOutDefaults);
