...
import { _firebaseAppFactory, runOutsideAngular } from '@angular/fire';
;
var Value = (function () {
...
var AngularFireRemoteConfig = (function () {
    function AngularFireRemoteConfig(options, nameOrConfig, settings, defaultConfig, zone) {
        this.zone = zone;
        var remoteConfig$ = of(undefined).pipe(switchMap(function () { return zone.runOutsideAngular(function () { return import('firebase/remote-config'); }); }), map(function () { return _firebaseAppFactory(options, zone, nameOrConfig); }), map(function (app) { return app.remoteConfig(); }), tap(function (rc) {
            if (settings) {
        ...
        }), startWith(undefined), runOutsideAngular(zone), shareReplay({ bufferSize: 1, refCount: false }));
        ...
        var existing$ = loadedRemoteConfig$.pipe(switchMap(function (rc) { return rc.activate().then(function () { return rc.getAll(); }); }), filterOutDefaults);
        var fresh$ = loadedRemoteConfig$.pipe(switchMap(function (rc) { return zone.runOutsideAngular(function () { return rc.fetchAndActivate().then(function () { return rc.getAll(); }); }); }), filterOutDefaults);
