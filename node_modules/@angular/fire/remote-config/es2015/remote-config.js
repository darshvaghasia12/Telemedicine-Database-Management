...
let AngularFireRemoteConfig = class AngularFireRemoteConfig {
    constructor(options, nameOrConfig, settings, defaultConfig, zone) {
        this.zone = zone;
        const remoteConfig$ = of(undefined).pipe(switchMap(() => zone.runOutsideAngular(() => import('firebase/remote-config'))), map(() => _firebaseAppFactory(options, zone, nameOrConfig)), map(app => app.remoteConfig()), tap(rc => {
            if (settings) {
        ...
        const existing$ = loadedRemoteConfig$.pipe(switchMap(rc => rc.activate().then(() => rc.getAll())), filterOutDefaults);
        const fresh$ = loadedRemoteConfig$.pipe(switchMap(rc => zone.runOutsideAngular(() => rc.fetchAndActivate().then(() => rc.getAll()))), filterOutDefaults);
